...
import { ExternalLink, Loader2, Plus, Trash2, CheckCircle2, Clock, XCircle } from "lucide-react";
...
export function LinkManager({
  ...
  studentId?: string;
  initialLinks?: ProjectLinkDto[];
  onChanged?: () => void;
}) {
  const [links, setLinks] = useState<ProjectLinkDto[]>(initialLinks ?? []);
  ...
  return (
    <Card className="rounded-md border border-[#e2ded5] bg-white p-4 dark:border-[#262f3c] dark:bg-[#1b222c]">
      <CardContent className="space-y-4 p-0">
        <div>
          <h3 className="text-sm font-bold text-[#1c2024] dark:text-white">Project &amp; Live Deployment Links</h3>
          <p className="mt-0.5 text-xs text-[#5c6470] dark:text-[#94a3b8]">
            Deployed links, GitHub code repositories, and in-house laboratory projects for coordinator audit.
          </p>
        </div>
        ...
        {links.length === 0 ? (
          <p className="py-4 text-center text-xs text-[#5c6470] dark:text-[#94a3b8]">
            No project links submitted yet.
          </p>
        ) : (
