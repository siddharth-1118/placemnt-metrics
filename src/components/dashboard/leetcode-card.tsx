  ...
  return (
    <Card className="rounded-md border border-[#e2ded5] bg-white p-4 dark:border-[#262f3c] dark:bg-[#1b222c]">
      ...
      <CardContent className="space-y-4 p-0 pt-3">
        ...
        {/* Recognitions strip: top skill tags + contest rating if any */}
        {(data.skillTags.length > 0 || data.contestRating) && (
          <div>
            <p className="mb-1.5 text-xs font-semibold text-[#1c2024] dark:text-white">
              Strengths &amp; Recognitions ({data.skillTags.length})
            </p>
            <div className="flex flex-wrap gap-1.5">
              {data.contestRating && (
                <span className="rounded border border-emerald-300 bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300">
                  Contest rating {Math.round(data.contestRating)}
                </span>
              )}
              {data.skillTags.slice(0, 8).map((b) => (
                <span
                  key={b.name}
